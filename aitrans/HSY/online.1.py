...
# import ABRold as ABR
def test(user_id):
    history = pd.DataFrame(columns = ["time_interval", "send_data_size", 'frame_time_len', 'frame_type', 'real_quality',  "buffer_size", 'end_delay', 'rebuf_time', 'cdn_flag', 'buffer_flag', 'bitrate', 'buffer', 'reward', 'reward_sum'])
    # path setting
    ...
    video_id = random.randint(0, 5)
    video_size_file = './video_trace/'+idDict[video_id]+'/frame_trace_'      #video trace path setting,
    ...
    while True:
        ...
        if call_time > 0.5 and not end_of_video:
            ...
            try:
                history.iloc[-1, 12] = reward_frame
                history.iloc[-1, 13] = reward_sum
            except:
                print("This is the first record.")

            bit_rate, target_buffer = random.choice(bit_rates), random.choice(buffers)+0.5
            
            history = pd.concat([history, pd.DataFrame( [[S_time_interval[-1], S_send_data_size[-1], S_frame_time_len[-1], S_frame_type[-1], S_real_quality[-1],  S_buffer_size[-1], S_end_delay[-1], rebuf_time, cdn_flag, buffer_flag, bit_rate, target_buffer, 0, 0]], columns = ["time_interval", "send_data_size", 'frame_time_len', 'frame_type', 'real_quality',  "buffer_size", 'end_delay', 'rebuf_time', 'cdn_flag', 'buffer_flag', 'bitrate', 'buffer', 'reward', 'reward_sum']) ])
    ...
    history.to_csv("./data_csv/t_{}.csv".format(id)) #这里加一个文件夹路径吧，你看着办
    return reward_all_sum

allName = sorted(os.listdir("./video_trace/"))
allName.remove(".DS_Store")
idDict = dict(zip(range(0, len(allName)), allName))

for id in range(int(input("start id:")), int(input("end id:"))):
    a = test("aaa")
    print(a)