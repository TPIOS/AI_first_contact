...
def test(user_id):
    ...
    while True:
        ...
        if call_time > 0.5 and not end_of_video:
            ...
            bit_rate, target_buffer = random.choice(bit_rates), random.choice(buffers)
    ...
    history.to_csv("t_{}.csv".format(id)) #这里加一个文件夹路径吧，你看着办
    return reward_all_sum

# id用处见上两行，注意，每次随机分配目录，id不要重复，多个进程用错开的id
